...
plantgrow.protected = tonumber(minetest.setting_get("mobs_spawn_protected")) or 1
plantgrow.remove = minetest.setting_getbool("remove_far_mobs")
...
function plantgrow:register_mob(name, def)
		...
		if self.attack_type == "explode" then
			...
			if dist > self.reach then
				...
				if self.timer > 3 then
					...
					plantgrow:explosion(pos, 2, 0, 1, self.sounds.explode)
...
end -- END plantgrow:register_mob function

-- global functions

plantgrow.spawning_mobs = {}

function plantgrow:spawn_specific(name, nodes, neighbors, min_light, max_light,
	interval, chance, active_object_count, min_height, max_height)

	plantgrow.spawning_mobs[name] = true
	...
	if new_chance ~= nil then
		chance = new_chance
		print ("[plantgrow Redo] Chance setting for " .. name .. " is now " .. chance)
	end
	...
	minetest.register_abm({
		...
		action = function(pos, node, _, active_object_count_wider)
			...
			or not plantgrow.spawning_mobs[name] then
				return
			end
			...
			if mob == false then
				print ("[plantgrow]" .. name .. " failed to spawn at "
				.. minetest.pos_to_string(pos))
			else
--				print ("[plantgrow] Spawned " .. name .. " at "
...
function plantgrow:register_spawn(name, nodes, max_light, min_light, chance, active_object_count, max_height)

	plantgrow:spawn_specific(name, nodes, {"air"}, min_light, max_light, 30,
...
function plantgrow:explosion(pos, radius, fire, smoke, sound)
...
function plantgrow:register_arrow(name, def)
...
-- Spawn Egg
function plantgrow:register_egg(mob, desc, background, addegg)
...
function plantgrow:capture_mob(self, clicker, chance_hand, chance_net, chance_lasso, force_take, replacewith)
	...
	and clicker:get_inventory() then
		...
		if clicker:get_inventory():room_for_item("main", mobname) then
			...
			elseif tool:get_name() == "plantgrow:net" then
			...
			elseif tool:get_name() == "plantgrow:magic_lasso" then
...
function plantgrow:feed_tame(self, clicker, feed_count, breed, tame)
